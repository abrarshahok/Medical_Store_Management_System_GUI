...
void MainWindow::on_admin_login_button_clicked()
{
    ...
    else if (ui->admin_name_input->text() == "admin" && ui->admin_passw_input->text() == "admin")
    {
        QMessageBox::information(this, "Success", "Login Successful");
        ui->Medical_Store->setCurrentIndex(3);
        MainWindow::on_see_tab_button_clicked();
...
void MainWindow::on_update_price_button_clicked()
{
    ...
    {
        QMessageBox::warning(this, "Error", "Price Cannot be <= 0");
        return;
    }

    if (ui->new_price_input->text().isEmpty())
    {
        QMessageBox::warning(this, "Error", "Please Enter Tab Price to Update Price");
        return;
    }
...
void MainWindow::on_user_login_button_clicked()
{
    ...
    else if (ui->user_input->text() == "local" && ui->user_pass_input->text() == "local")
    {
        QMessageBox::information(this, "Success", "Login Successful");
        ui->Medical_Store->setCurrentIndex(4);
        ui->run_store_features->setCurrentIndex(0);
...
void MainWindow::on_See_Cart_Page_Button_clicked()
{
    ...
    if (show_cart.exec())
    {
        ...
        while (show_cart.next())
        {
            ...
            ui->see_cart_Table->setItem(rowNum, 4, new QTableWidgetItem("   " + QString(show_cart.value("Bill_Date").toString())));
            ui->see_cart_Table->setItem(rowNum, 5, new QTableWidgetItem("   " + QString(show_cart.value("Bill_Time").toString())));
            rowNum++;
        }
    }
